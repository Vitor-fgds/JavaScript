...
botao.addEventListener("click", function(){
    ...
    mensagem = `Calculando a média final de ${nome} <br> As notas obtidas foram ${nota1} e ${nota2} <br> A média final será ${(nota1+ nota2) / 2} <br>`
    adicionaMensagem(mensagem);

    if ((nota1 + nota2)/2 > 6){
        let parabens = `A mensagem que temos é: MEUS PARABÉNS!`;
        adicionaMensagem(parabens)
    }
    else{
        let parabens = `A mensagem que temos é: ESTUDE UM POUCO MAIS!`
        adicionaMensagem(parabens)
    }
